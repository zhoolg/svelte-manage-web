...
function browserClassName(name: string): string {
...
function showUnsupportedWarning(browserInfo: BrowserInfo): void {
  ...
  unsubscribe();
  const browserParams = {
    browser: escapeHtml(browserInfo.name),
    version: escapeHtml(browserInfo.version),
  };
  ...
  const modal = `
    ...
    ">
      <div style="
        ...
        width: 90%;
        text-align: center;
        box-shadow: 0 20px 60px color-mix(in srgb, var(--color-primary) 20%, transparent), 0 0 0 1px rgba(255, 255, 255, 0.1);
      ...
      ">
        ...
        ">
          ${escapeHtml(currentT('browserCompatibility.unsupported.title'))}
        </h2>
        ...
        <p style="
          ...
          margin: 0 0 24px 0;
        ">
          ${escapeHtml(currentT('browserCompatibility.unsupported.description', browserParams))}
        </p>
        ...
        <p style="
          ...
          margin: 0 0 24px 0;
        ">
          ${escapeHtml(currentT('browserCompatibility.unsupported.upgradeHint'))}
        </p>
        ...
        <div style="
          ...
          justify-content: center;
        ">
          <a href="https://www.google.com/chrome/" target="_blank" rel="noopener noreferrer" style="
            flex: 1;
            padding: 14px 24px;
            background: linear-gradient(135deg, var(--color-primary) 0%, color-mix(in srgb, var(--color-primary) 65%, white) 100%);
            color: white;
          ...
          ">
            ...
            ${escapeHtml(currentT('browserCompatibility.unsupported.downloadChrome'))}
          </a>
          <a href="https://www.mozilla.org/firefox/" target="_blank" rel="noopener noreferrer" style="
            flex: 1;
            padding: 14px 24px;
          ...
          "
          onmouseover="
            this.style.background='color-mix(in srgb, var(--color-primary) 12%, transparent)';
            this.style.borderColor='color-mix(in srgb, var(--color-primary) 28%, transparent)';
            this.style.transform='translateY(-2px)';
          "
          onmouseout="
            this.style.background='color-mix(in srgb, var(--color-primary) 8%, transparent)';
            this.style.borderColor='color-mix(in srgb, var(--color-primary) 20%, transparent)';
            this.style.transform='translateY(0)';
          ">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 8px; vertical-align: middle;"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>
            ${escapeHtml(currentT('browserCompatibility.unsupported.downloadFirefox'))}
          </a>
        </div>
      </div>
    </div>
  ...
  const message = `
    ...
    ">
      <div style="max-width: 1200px; margin: 0 auto; display: flex; align-items: center; justify-content: space-between;">
        <span>
          ${escapeHtml(currentT('browserCompatibility.oldVersion.message', browserParams))}
        </span>
        <button onclick="this.parentElement.parentElement.remove()" style="
          ...
          white-space: nowrap;
        ">
          ${escapeHtml(currentT('browserCompatibility.oldVersion.dismiss'))}
        </button>
      </div>
    </div>
  `;
...
export function initBrowserCompatibility(): void {
  ...
  if (browserInfo.isOldVersion) {
    ...
    document.documentElement.classList.add(`old-${browserClassName(browserInfo.name)}`);
    console.log(
      '[Browser] Added classes:',
      'old-browser',
      `old-${browserClassName(browserInfo.name)}`
    );
  }
