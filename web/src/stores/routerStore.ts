...
// 当前页面刷新版本号，用于应用内重挂载当前路由内容。
export const routeRefreshVersion = writable(0);
...
export function refreshCurrentRoute() {
  routeRefreshVersion.update(version => version + 1);
}
 ...
 * 这样新增模块时无需手动添加路由配置
 */
function getRouteNames(): Record<string, string> {
...
export const routeNames: Record<string, string> = getRouteNames();
...
export const currentRouteName = derived(currentPath, $path => {
  ...
  const key = getRouteNames()[cleanPath] || 'menu.home';
  return t(key);
});
