...
<script lang="ts">
  ...
  export const confirm = (options: ConfirmOptions | string): Promise<boolean> => {
    const config: ConfirmOptions = typeof options === 'string' ? { content: options } : options;
    return new Promise(resolve => {
      confirmState.set({ ...config, visible: true, resolve });
    });
  };
  ...
  function getTypeConfig(type?: string) {
    switch (type) {
      ...
      default:
        return { icon: Info, color: 'var(--color-primary)' };
    }
  }
    ...
    >
      ...
      >
        <Button.Root
          ...
          class="h-8 px-4 text-sm border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 rounded hover:text-[color:var(--color-primary)] hover:border-[color:var(--color-primary)] transition-colors"
        >
        ...
        <Button.Root
          ...
          class="h-8 px-4 text-sm text-white rounded transition-colors {$confirmState.type ===
          'danger'
            ? 'bg-[#f56c6c] hover:bg-[#f78989]'
            : 'bg-[color:var(--color-primary)] hover:bg-[color:color-mix(in_srgb,var(--color-primary)_85%,white)]'}"
        >
