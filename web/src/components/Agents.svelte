<script lang="ts">
  ...
  $: filteredAgents = agents.filter(
    a => a.name.includes(searchKeyword) || a.contact.includes(searchKeyword)
  );
  ...
  function getStatusColor(status: string) {
    switch (status) {
      ...
      case 'pending':
        return 'bg-yellow-50 text-yellow-600 dark:bg-yellow-900/30';
      case 'rejected':
        return 'bg-red-50 text-red-600 dark:bg-red-900/30';
      default:
  ...
  <div class="bg-white dark:bg-[#1d1d1d] rounded-lg shadow-sm p-4">
    <div class="flex items-center gap-4">
      <input
        ...
        class="flex-1 h-9 px-3 text-sm border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-[#141414] focus:border-[color:var(--color-primary)] focus:outline-none"
      />
      <Button.Root
      ...
      <Button.Root
        class="h-9 px-4 bg-[#67c23a] text-white text-sm rounded-lg hover:bg-[#85ce61] transition-colors flex items-center gap-1"
      >
        <Plus size={14} />
        {$t('agent.addAgent')}
      </Button.Root>
          ...
          <tr class="hover:bg-gray-50 dark:hover:bg-[#262626] transition-colors">
            ...
            <td class="px-4 py-3 text-sm">
              <Button.Root class="text-[#409eff] hover:text-[#66b1ff] mr-3"
                >{$t('common.view')}</Button.Root
              >
              {#if agent.status === 'pending'}
                <Button.Root class="text-[#67c23a] hover:text-[#85ce61] mr-3"
                  >{$t('agent.approve')}</Button.Root
                >
                <Button.Root class="text-red-500 hover:text-red-600"
                  >{$t('agent.reject')}</Button.Root
                >
              {:else}
                <Button.Root class="hover:opacity-80" style="color: var(--color-primary);"
