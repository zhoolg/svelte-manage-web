...
>
  <div class="relative w-full">
    <div
      class="flex h-9 w-full items-center rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 text-sm focus-within:border-[color:var(--color-primary)] focus-within:ring-1 focus-within:ring-[color:var(--color-primary)] transition-all {disabled
    ...
    >
          ...
          {#each segments as { part, value: segValue }}
            ...
            {:else}
              <DateRangePicker.Segment
                {part}
                class="rounded px-0.5 focus:bg-[color:var(--color-primary)] focus:text-white focus:outline-none"
              >
                {segValue}
              </DateRangePicker.Segment>
            {/if}
      ...
      <DateRangePicker.Trigger
        class="ml-auto pl-2 text-gray-400 hover:text-[color:var(--color-primary)] transition-colors"
      >
        <Calendar size={14} />
      </DateRangePicker.Trigger>
    </div>
                        ...
                        <DateRangePicker.Cell {date} month={month.value} class="p-0">
                          <DateRangePicker.Day
                            class="w-9 h-9 flex items-center justify-center text-sm rounded transition-colors
                              ...
                              data-[today]:text-[color:var(--color-primary)] data-[today]:font-semibold
                              data-[selected]:bg-[color:var(--color-primary)] data-[selected]:text-white
                              data-[selection-start]:rounded-l-md data-[selection-start]:bg-[color:var(--color-primary)] data-[selection-start]:text-white
                              data-[selection-end]:rounded-r-md data-[selection-end]:bg-[color:var(--color-primary)] data-[selection-end]:text-white
                              data-[highlighted]:bg-[color:color-mix(in_srgb,var(--color-primary)_10%,transparent)]
