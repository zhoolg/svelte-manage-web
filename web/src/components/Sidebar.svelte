<script lang="ts">
  ...
  import { getMenuConfig, hasActiveChild, type MenuItem } from '../config/menu';
  ...
  let menuConfig = $state<MenuItem[]>(getMenuConfig());
  ...
  function releaseSidebarFocus() {
    if (typeof document === 'undefined' || !sidebarElement) {
      return;
    }
    const activeElement = document.activeElement;
    if (activeElement instanceof HTMLElement && sidebarElement.contains(activeElement)) {
      activeElement.blur();
    }
  }
...
<aside
  ...
  class="fixed top-0 left-0 h-screen transition-all duration-300 z-50 bg-[var(--sidebar-bg)] shadow-2xl md:shadow-none"
  class:w-[54px]={collapsed && !mobile}
  class:w-[210px]={!collapsed || mobile}
  class:-translate-x-full={mobile && !open}
