...
<div class="space-y-2">
  ...
  <div class="flex flex-wrap gap-4">
    ...
    {#if !disabled && ((!multiple && imageList.length === 0) || (multiple && (limit === 0 || imageList.length < limit)))}
      <button
        type="button"
        class="w-32 h-32 border border-dashed border-gray-300 dark:border-gray-600 rounded flex flex-col items-center justify-center gap-2 text-gray-400 hover:border-[color:var(--color-primary)] hover:text-[color:var(--color-primary)] cursor-pointer transition-colors bg-gray-50 dark:bg-gray-800/50"
        onclick={triggerUpload}
