<script lang="ts">
  ...
  $: isCompact = $settingsStore.density === 'compact';
</script>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      <div>
        ...
        <input
          ...
          class="w-full h-9 px-3 text-sm border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-[#141414] text-gray-800 dark:text-white focus:outline-none focus:border-[color:var(--color-primary)]"
        />
      </div>
      <div>
      ...
      <div class="flex items-end gap-2">
        <Button.Root
          ...
          class="h-9 px-4 text-white text-sm rounded-lg transition-colors flex items-center gap-1 hover:brightness-105 active:brightness-95"
        ...
        <Button.Root
          onclick={handleReset}
          class="h-9 px-4 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 text-sm rounded-lg transition-colors flex items-center gap-1 hover:text-[color:var(--color-primary)] hover:border-[color:var(--color-primary)]"
        >
          <RefreshCw size={14} />
          {$t('common.reset')}
        </Button.Root>
  ...
  <div class="bg-white dark:bg-[#1d1d1d] rounded-lg shadow-sm overflow-hidden">
          ...
          {#each logs as log}
            <tr class="hover:bg-gray-50 dark:hover:bg-[#262626] transition-colors">
              <td
                class="px-4 {isCompact
                  ? 'py-2 text-xs'
                  : 'py-3 text-sm'} text-gray-600 dark:text-gray-400"
              >
                {log.id}
              </td>
              <td
                class="px-4 {isCompact
                  ? 'py-2 text-xs'
                  : 'py-3 text-sm'} text-gray-800 dark:text-white"
              >
                {log.username}
              </td>
              <td class="px-4 {isCompact ? 'py-2 text-xs' : 'py-3 text-sm'}">
                <span
              ...
              <td
                class="px-4 {isCompact
                  ? 'py-2 text-xs'
                  : 'py-3 text-sm'} text-gray-600 dark:text-gray-400 font-mono"
              >
                {log.ip}
              </td>
              <td
    ...
    >
      ...
      <div class="flex items-center gap-2">
        <Button.Root
          class="px-3 py-1 text-sm border border-gray-200 dark:border-gray-700 rounded transition-colors disabled:opacity-50 hover:text-[color:var(--color-primary)] hover:border-[color:var(--color-primary)]"
          disabled
        >
          {$t('table.prev')}
        </Button.Root>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative w-full max-w-lg mx-4 bg-white dark:bg-[#1d1d1d] rounded-lg shadow-xl">
        ...
        <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-800 flex justify-end">
          <Button.Root
            onclick={() => (selectedLog = null)}
            class="px-4 py-2 text-sm border border-gray-200 dark:border-gray-700 rounded-lg transition-colors hover:text-[color:var(--color-primary)] hover:border-[color:var(--color-primary)]"
          >
            {$t('common.close')}
          </Button.Root>
        </div>
