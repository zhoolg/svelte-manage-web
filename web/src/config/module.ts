...
export interface DynamicLabel {
  /** 多语言显示文案，运行时模块由后端 metadata 下发 */
  labelI18n?: LocaleTextMap;
}

// ==================== 表格列配置 ====================

export interface TableColumn<T = Record<string, unknown>> extends DynamicLabel {
  /** 字段名 */
  ...
  statusMap?: Record<
    string | number,
    { label: string; color: string; text?: string; labelI18n?: LocaleTextMap }
  >;
...
export interface SearchField {
  ...
  placeholder?: string;
  /** 多语言占位符 */
  placeholderI18n?: LocaleTextMap;
  /** 选项（select/radio 类型使用） */
  options?: Array<{ label: string; value: string | number; labelI18n?: LocaleTextMap }>;
  /** 默认值 */
...
export interface ActionButton<T = Record<string, unknown>> extends DynamicLabel {
  /** 内置动作标识 */
  ...
  /** 工作流状态字段，预览模式用于本地模拟流转 */
  workflowStatusField?: string;
  /** 工作流目标状态，预览模式用于本地模拟流转 */
  workflowTo?: string | number;
}
