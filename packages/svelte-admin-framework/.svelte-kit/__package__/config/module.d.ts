...
export type FormFieldType = 'input' | 'textarea' | 'select' | 'radio' | 'checkbox' | 'switch' | 'number' | 'date' | 'datetime' | 'upload' | 'editor' | 'tags';
 ...
 * 创建模块配置（带类型推断）
 */
export declare function defineModule<T = Record<string, unknown>>(config: ModuleConfig<T>): ModuleConfig<T>;
